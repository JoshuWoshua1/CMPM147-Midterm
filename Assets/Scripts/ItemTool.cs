...
public class ItemTool : MonoBehaviour
{
    ...
    private ItemNameData ParseNameData(string jsonContent)
    {
        try
        {
            ItemNameData manualParsed = ParseNameDataWithNewtonsoft(jsonContent);
            if (IsNameDataValidShape(manualParsed))
            {
                return manualParsed;
            }
        }
        ...
        {
            // Ignore and fall back to JsonUtility parsing.
        }
    ...
    private ItemNameData ParseNameDataWithNewtonsoft(string jsonContent)
    {
        Newtonsoft.Json.Linq.JObject root = Newtonsoft.Json.Linq.JObject.Parse(jsonContent);
        List<string> baseNames = root["baseNames"]?.ToObject<List<string>>();
        List<BaseNameCategory> categories = root["categories"]?.ToObject<List<BaseNameCategory>>();
        ModifierDefinition[] prefix = (root["prefix"] ?? root["prefixModifiers"])?.ToObject<ModifierDefinition[]>();
        ModifierDefinition[] suffix = (root["suffix"] ?? root["suffixModifiers"])?.ToObject<ModifierDefinition[]>();
